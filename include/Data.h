...
// means treatment.
// The current implemenation also do not allow missing data. We assume to preprocess the data before applying the methods.
// The unique number of any nominal categorial variable should be less or equal to 31.
    class Data {
    public:
        // Constructor
    ...
    private:
        ...
        // We focus on the case of developing one optimal action each time so that action is a vector.
        //uint32_t is used so that each time it can pack 8 actions and each action takes 4 bits.
